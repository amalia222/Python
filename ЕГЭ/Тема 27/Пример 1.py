...
clusters = clusters[:3]
print(int(distance(cluster_center(clusters[0]), cluster_center(clusters[-1])) * 10000))

total_max_dist = 0
for cluster in clusters:
    center = cluster_center(cluster)
    max_dist = max(distance(center, point) for point in cluster)
    total_max_dist = max(total_max_dist, max_dist)
print(int(total_max_dist * 10000))
